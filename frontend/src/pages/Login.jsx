...
import { loginApi, useApi } from "../api";
...
export default function Login() {
  ...
  const formSubmit = async (e) => {
    ...
    try {
      ...
      // res expected shape: { success: true/false, message: "...", token: "...", user: {...} }
      if (res && res.success) {
        // save token/user as provided by backend
        if (res.token) {
          setToken(res.token);
          localStorage.setItem("token", res.token);
        }
        if (res.user) localStorage.setItem("user", JSON.stringify(res.user));
        // optional: save entire response
        ...
        <div className="flex w-full max-w-md flex-col items-center gap-6">
          ...
          >
            ...
            <div className="mt-8 flex w-full flex-col gap-4">
              ...
              <div className="flex w-full flex-col">
                <div className="flex items-center justify-between">
                  <label className="pb-2 text-sm font-medium" htmlFor="password" style={{ color: col("#1f2937", "#d1d5db") }}>
                    Password
                  </label>
                  <a className="text-sm font-medium hover:underline" style={{ color: colors.primary }} href="#">
                    Forgot Password?
                  </a>
                </div>
            ...
            <button
              ...
              style={{ background: colors.primary, color: "#fff", opacity: loading ? 0.7 : 1 }}
            >
            ...
            <p className="mt-8 text-center text-sm" style={{ color: col("#6b7280", "#9ca3af") }}>
              ...
              <a className="font-semibold hover:underline" style={{ color: colors.primary }} href="#">
                Sign Up
              </a>
            </p>
          </div>
